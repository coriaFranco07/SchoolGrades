...
if (!empty($_POST["nombre"]) && !empty($_POST["apellido"]) && !empty($_POST["correo"]) && !empty($_POST["usuario"]) && !empty($_POST["contraseña"]) && !empty($_POST["genero"]) && !empty($_POST["estado"])) {
    ...
    $sql = "UPDATE profesores SET nombre = '$nombre', apellido = '$apellido', correo = '$correo', usuario = '$usuario', contraseña = '$contraseña', id_sexo = $genero, id_estado = $estado WHERE id_profesor = $id";
